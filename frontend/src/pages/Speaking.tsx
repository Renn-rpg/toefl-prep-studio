    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-20 h-20">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="font-mono font-bold text-lg text-stone-800">{score}</span>
        </div>
      </div>
...
export function Speaking() {
  ...
  if (!selected) return (
    <div className="max-w-2xl space-y-6 animate-fade-up">
      <div>
        <h1 className="font-display text-3xl font-bold text-stone-800">口语练习</h1>
        <p className="text-stone-500 mt-1 text-sm">录制回答，获取 AI 三维评分与详细反馈</p>
      </div>
      <div className="space-y-3">
        {prompts.map(p => (
          <button key={p.id} onClick={() => setSelected(p)}
            ...
            <p className="text-sm text-stone-700">{p.prompt}</p>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="max-w-2xl space-y-6 animate-fade-up">
      <button onClick={resetAll} className="flex items-center gap-1.5 text-sm text-teal-600 hover:text-teal-700">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
        ...
        <p className="mt-3 font-medium text-stone-800 leading-relaxed">{selected.prompt}</p>
      </div>
      ...
      {!result ? (
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-4">
            ...
            {state === 'recording' && (
              <button onClick={stop} className="flex items-center gap-2 bg-stone-800 hover:bg-stone-900 text-white rounded-full px-5 py-2 text-sm font-medium animate-pulse">
                <MicOff className="h-4 w-4" /> 停止录音
              </button>
            )}
            {state === 'stopped' && (
              <button onClick={reset} className="flex items-center gap-2 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-full px-4 py-2 text-sm">
          ...
          <div>
            <label className="block text-xs font-medium text-stone-500 mb-1.5">
              转录文本 <span className="font-normal text-stone-400">（填写你说的内容，用于 AI 评分）</span>
            </label>
            <textarea rows={5} value={transcript} onChange={e => setTranscript(e.target.value)}
              className="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm resize-none bg-stone-50 focus:outline-none focus:ring-2 focus:ring-teal-400"
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm space-y-5">
          <div>
            ...
            <p className="text-sm text-stone-400 mt-0.5">{feedback.band_descriptor}</p>
          </div>
          <div className="flex justify-around py-4">
            <ScoreRing score={result.pronunciation_score} label="发音" color="#0F766E" />
          ...
          <button onClick={resetAll} className="text-sm text-teal-600 hover:underline">重新练习</button>
        </div>
      )}
    </div>
  )
