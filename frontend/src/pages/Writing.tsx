...
export function Writing() {
  ...
  if (!selected) return (
    <div className="max-w-3xl space-y-6 animate-fade-up">
      <div>
        <h1 className="font-display text-3xl font-bold text-slate-800">写作练习</h1>
        <p className="text-slate-500 mt-1 text-sm">AI 从任务达成、连贯性、语言三个维度进行评分</p>
      </div>
  ...
  return (
    <div className="max-w-3xl space-y-6 animate-fade-up">
      <button onClick={() => setSelected(null)} className="flex items-center gap-1.5 text-sm text-blue-600">
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-medium text-slate-500">论文</label>
            ...
            <textarea rows={14} value={essay} onChange={e => setEssay(e.target.value)}
              className="w-full border border-slate-200 rounded-2xl px-5 py-4 text-sm resize-none bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 leading-relaxed"
              placeholder="在此开始写作..." />
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-slate-400">{feedback.band_descriptor}</p>
              </div>
              <div className="font-mono text-4xl font-bold text-blue-600">
                {result.total_score}<span className="text-lg text-slate-400">/90</span>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3 mb-5">
              {[
                { label: '任务达成', value: result.task_achievement_score, color: 'text-blue-600' },
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-emerald-50 rounded-xl p-4">
                ...
                <ul className="space-y-1">{feedback.strengths.map((s, i) => (
                  <li key={i} className="text-sm text-slate-600 flex gap-1.5"><span className="text-emerald-500">✓</span>{s}</li>
                ))}</ul>
              </div>
              <div className="bg-amber-50 rounded-xl p-4">
                <p className="text-xs font-semibold text-amber-700 mb-2">改进建议</p>
                <ul className="space-y-1">{feedback.suggestions.map((s, i) => (
                  <li key={i} className="text-sm text-slate-600 flex gap-1.5"><span className="text-amber-500">→</span>{s}</li>
                ))}</ul>
              </div>
            </div>
            {feedback.corrected_excerpt && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <p className="text-xs font-semibold text-blue-700 mb-1">修改示例</p>
                <p className="text-sm text-slate-700 italic">"{feedback.corrected_excerpt}"</p>
              </div>
            )}
          </div>
          <button onClick={() => setResult(null)} className="text-sm text-blue-600 hover:underline">修改文章</button>
        </div>
      )}
    </div>
  )
